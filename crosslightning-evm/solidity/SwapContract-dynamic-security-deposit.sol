...
contract CrossLightningSwaps {
    ...
    */
    ...
    function offerer_claimInit(AtomicSwapStruct memory payReq, Signature calldata signature, bytes32 txoHash) payable external returns(bytes32) {
        require(payReq.claimerBounty==0 && payReq.securityDeposit==0, "Invalid deposit");
    ...
    function offerer_init(AtomicSwapStruct memory payReq, Signature calldata signature, bytes32 txoHash) payable external returns(bytes32) {
        require(msg.value>=payReq.claimerBounty && msg.value>=payReq.securityDeposit, "Invalid amount deposited");
    ...
    function payOutClaimerBounty(AtomicSwapStruct memory payReq, address initiator) internal {
        ...
        if(securityDeposit>0) payable(payReq.claimer).transfer(securityDeposit);
    }
    ...
    function payOutSecurityDepositClaimerOnly(AtomicSwapStruct memory payReq) internal {
        uint256 securityDeposit = payReq.securityDeposit;
        uint256 claimerBounty = payReq.claimerBounty;
        if(securityDeposit>0) {
            if(claimerBounty<=securityDeposit) return;
    ...
    function offerer_multi_refund(AtomicSwapStruct[] memory payReqs, bool payIn) public {
        ...
        for(uint i=0;i<payReqs.length;i++) {
            ...
            if(!isPayIn && amountRefunded>0) securityDeposits += payReqs[i].securityDeposit;
        ...
        if(securityDeposits>0) payable(msg.sender).transfer(securityDeposits);
